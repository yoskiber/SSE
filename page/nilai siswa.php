...
if ($_SERVER["REQUEST_METHOD"] == "POST" AND isset($_POST["save"])) {
	...
	if ($validasi) {
		foreach ($_POST["nilai"] as $kd_kriteria => $nilai) {
			...
			if ($q->num_rows) {
				echo alert("Nilai untuk ".$_POST["nim"]." sudah ada!", "?page=nilai siswa");
				$err = true;
			}
		}
	}
  ...
  if (!$err AND $connection->query($sql)) {
		echo alert("Berhasil!", "?page=nilai siswa");
	} else {
		echo alert("Gagal!", "?page=nilai siswa");
	}
}
	    ...
	    <div class="panel panel-<?= ($update) ? "warning" : "info" ?>">
	        ...
	        <div class="panel-body">
	            <form action="<?=$_SERVER["REQUEST_URI"]?>" method="post">
	                ...
	                <?php if ($update): ?>
										<a href="?page=nilai siswa" class="btn btn-info btn-block">Batal</a>
									<?php endif; ?>
	            </form>
	        </div>
	    </div>
