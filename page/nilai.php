							...
							<label for="nim">siswa</label>
										...
										<?php else: ?>
											<select class="form-control" name="nim">
												<option>---</option>
												<?php $sql = $connection->query("SELECT * FROM mahasiswa WHERE nim IN(SELECT nim FROM mahasiswa) AND status = 'Diterima'"); while ($data = $sql->fetch_assoc()): ?>
	    ...
	    <div class="panel panel-info">
	        ...
	        <div class="panel-body">
	            <table class="table table-condensed">
	                ...
	                <tbody>
	                   <?php $no = 1; ?>
	                    <?php if ($query = $connection->query("SELECT * FROM mahasiswa WHERE nim IN(SELECT nim FROM mahasiswa) AND status = 'Diterima'")): ?>
